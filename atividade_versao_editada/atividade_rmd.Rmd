...
Este relatório[@villar2022] tem como objetivo apresentar funcionalidades do R Markdown, utilizando dados públicos sobre barragens de mineração no Brasil.
...
```{r plot-dpa, out.width="100%", dpi = 300, fig.cap="Gráfico ..."}
dados_sigbm_filtrados |>
  ...
  labs(x = "Dano potencial associado (DPA)", y = "Quantidade de barragens",
       title = glue::glue(
    "Dano potencial associado de barragens de mineração em {params$uf}"
  ))
```
...
```{r tab-nivel-emergencia}
dados_sigbm_filtrados |> 
  count(dano_potencial_associado, sort = TRUE) |> 
  rename(`Dano potencial associado` = dano_potencial_associado, `Quantidade de barragens cadastradas` = n) |> 
  flextable::flextable() |> 
    flextable::autofit() |> 
  flextable::set_caption(glue::glue(
    "Dano potencial associado de barragens de mineração em {params$uf}"
  )) 
```
